...
mongoose.connect(config.mongoConnectionMain, {
...
async function simulateDM(m, messageToSend) {
    ...
    msg.react('✅')
    const filter = (reaction, user) => (user.id === m.author.id && reaction.emoji.name == '✅')
...
client.on('message', async (m) => {
    ...
    if (command == "end") {
        ...
        let msg = await m.channel.send("<a:Loading:851630056014741534> Updating Channel. This might take a while.")
    ...
    if (m.channel.id != config.mainChannelID && m.channel.id != config.queueChannelID && m.channel.id != config.commandsChannelID && m.channel.id != config.registerChannelID) return
    ...
    if (command == "register") {
        if (m.channel.id !== config.registerChannelID) {
            ...
            return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`That command goes in <#${config.registerChannelID}>`)).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`That command goes in <#${config.registerChannelID}>`)))
        }

        addAudit(`${m.author.id} tried to register with IGN: ${args[0]}`)
        if (m.author.id in await db.get(`data`)) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription("You are already registered"))
        if (args.length != 1) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
        ...
        message = await m.channel.send({component: new DiscButton.MessageActionRow()
                    .addComponents([new DiscButton.MessageButton()
                        .setLabel("Accept")
                        .setStyle("green")
                        .setID(`register-acc-${m.author.id}-${data.player.displayname}-${data.player.uuid}`),
    ...
    if (command == "join" || command == "j") {
        if (m.channel.id !== config.queueChannelID) {
            ...
            return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`That command goes in <#${config.queueChannelID}>`)).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.black)
        ...
        if (m.author.id in tagEloQueues) return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription("You are already queued")).catch((err) => simulateDM(m, new Discord.MessageEmbed().setColor(embedColors.black).setDescription("You are already queued")))
        if (m.author.id in tagEloGames) return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`You stil have a game against <@!${tagEloGames[m.author.id]}>`)).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`You stil have a game against <@!${tagEloGames[m.author.id]}>`)))
        if (m.author.id in tagEloDuels) return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`You stil have a game against <@!${tagEloDuels[m.author.id]}>`)).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`You stil have a game against <@!${tagEloDuels[m.author.id]}>`)))

        if (!(m.author.id in await db.get(`data`))) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription("You must be verified first. Do =register to start the verification process"))
        ...
        let potentialOpponents = await Object.entries(tagEloQueues).filter((a) => {return Math.abs(rating - a[1][0]) < deviation}).filter((a) => {return Math.abs(rating - a[1][0]) < a[1][1]}).sort((a, b) => {return (a[1][0] - rating) - (b[1][0] - rating)}).sort((a, b) => {return a[1][2] - b[1][2]})
        ...
        if (potentialOpponents.length == 0) {
            ...
            m.channel.send(new Discord.MessageEmbed()
                .setColor(embedColors.blue)
                .setDescription(`There ${Object.keys(tagEloQueues).length != 1 ? "are" : "is"} ${Object.keys(tagEloQueues).length} ${Object.keys(tagEloQueues).length != 1 ? "players" : "player"} in the queue`))
            return m.author.send(new Discord.MessageEmbed()
                .setColor(embedColors.green)
                .setDescription("Added you to the queue!")).catch((err) => simulateDM(m, new Discord.MessageEmbed()
                .setColor(embedColors.green)
        ...
        addAudit(`Starting match ${await db.get(`matches`)} ${potentialOpponents[0][0]} vs ${m.author.id}`)
        return m.channel.send(`<@!${m.author.id}> <@!${potentialOpponents[0][0]}>`, new Discord.MessageEmbed()
            .setColor(embedColors.blue)
            .setDescription(`Game ${await db.get(`matches`)} Starting:
<@!${potentialOpponents[0][0]}> ${await db.get(`data.${potentialOpponents[0][0]}.name`)} (${Math.round(potentialOpponents[0][1][0])}) vs <@!${m.author.id}> ${await db.get(`data.${m.author.id}.name`)} (${Math.round(rating)})`))
    }
        ...
        if (m.channel.id == config.commandsChannelID || m.channel.id == config.registerChannelID) return
        ...
        if (m.channel.id !== config.queueChannelID && m.channel.id !== config.mainChannelID) {
            ...
            return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`That command goes in <#${config.mainChannelID}>`)).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.black)
        ...
        .setDescription(`**${prefix}join** or **${prefix}j** - Join the ranked queue!
...
**${prefix}lb total** - See the most active players and your total games position!`)
        ...
        if (m.member.roles.cache.has('761106172186067015')) return m.channel.send("You are not authorized to use this")
        ...
        if (m.author.id in tagEloGames) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        .setDescription(`You are in a game against <@!${tagEloGames[m.author.id]}>. If you can't make it you must either forfeit (do ${prefix}forfeit <@!${tagEloGames[m.author.id]}>) or abort (do ${prefix}abort)`))
        if (m.author.id in tagEloDuels) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        .setDescription(`You are in a game against <@!${tagEloDuels[m.author.id]}>. If you can't make it you must either forfeit (do ${prefix}forfeit <@!${tagEloDuels[m.author.id]}>) or abort (do ${prefix}abort)`))


        if (!(m.author.id in tagEloQueues)) return m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription("You are not in a queue")).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.black)
        ...
        m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.blue)
            .setDescription(`There ${Object.keys(tagEloQueues).length != 1 ? "are" : "is"} ${Object.keys(tagEloQueues).length} ${Object.keys(tagEloQueues).length != 1 ? "players" : "player"} in the queue`))
        m.author.send(new Discord.MessageEmbed()
            .setColor(embedColors.red)
            .setDescription("Removed you from the queue")).catch((err) => simulateDM(m, new Discord.MessageEmbed()
            .setColor(embedColors.red)
        ...
        if (!(m.author.id in tagEloGames) && !(m.author.id in tagEloDuels)) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription("You are not in a game"))
        const msg = await m.channel.send(`<@!${m.author.id}> <@!${getOpponent(m.author.id)}>`, new Discord.MessageEmbed()
            .setColor(embedColors.blue)
        ...
        collector.on('end', (collection, reason) => {
            msg.reactions.removeAll()
            if (reason == "success") {
                if (m.author.id != getOpponent(getOpponent(m.author.id))) {return msg.edit({embed: new Discord.MessageEmbed()
                    .setColor(embedColors.black)
    ...
    else if (command == "resign" || command == "forfeit" || command == "ilost" || command == "ilose") {
        if (m.channel.id !== config.queueChannelID) {
            setTimeout(async function () {
                m.delete()
            }, 200);
        ...
        if (!(m.author.id in tagEloGames) && !(m.author.id in tagEloDuels)) return m.channel.send("You are not in a game")

        const msg = await m.channel.send(`<@!${getOpponent(m.author.id)}> <@!${m.author.id}>`, new Discord.MessageEmbed()
        .setColor(embedColors.blue)
        .setDescription(`React with a :thumbsup: if you consent that <@!${getOpponent(m.author.id)}> beat <@!${m.author.id}>`))
        ...
        collector.on('end', async (collection, reason) => {
            ...
            if (reason == "success") {
                console.log("1")
                if (!(m.author.id in tagEloGames) && !(m.author.id in tagEloDuels)) return msg.edit({embed: new Discord.MessageEmbed()
                    .setColor(embedColors.black)
                    .setDescription("Internal Error: Game End Failed! If this is an issue contact Mysterium")})
                let string;
        ...
        if (m.channel.id !== config.mainChannelID && m.channel.id !== config.commandsChannelID) {
            setTimeout(async function () {
                m.delete()
            }, 200);
            return m.author.send(new Discord.MessageEmbed()
        ...
        let player = mongoData.filter((a) => {return a.userID == checkingID})[0]
        ...
        for (let i = 0; i < player.records.length; i++) {
            ...
            let opponentRating = mongoData.filter((a) => {return a.userID == record.opponent})[0].elo
...
**Win Rate:** ${replaceError(Math.round(player.wins/(player.wins + player.losses)*1000)/10, 100)}%
...
// **Estimated Performance:** ${performanceScore}
        ...
        if (args[0].toLowerCase() == "host") {
            ...
            let data = Object.entries(await db.get('hostData')).filter((a) => {return a[1] > 0})
            if (!data.some((a) => {return a[0] == m.author.id})) data.push([m.author.id, 0])
            if (data.length < 10) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            ...
            let string = `${(position == 1) ? "**" : ""}#1 <@!${data[0][0]}> - ${data[0][1]}${(position == 1) ? "**" : ""}
${(position == 2) ? "**" : ""}#2 <@!${data[1][0]}> - ${data[1][1]}${(position == 2) ? "**" : ""}
${(position == 3) ? "**" : ""}#3 <@!${data[2][0]}> - ${data[2][1]}${(position == 3) ? "**" : ""}
${(position == 4) ? "**" : ""}#4 <@!${data[3][0]}> - ${data[3][1]}${(position == 4) ? "**" : ""}
${(position == 5) ? "**" : ""}#5 <@!${data[4][0]}> - ${data[4][1]}${(position == 5) ? "**" : ""}
${(position == 6) ? "**" : ""}#6 <@!${data[5][0]}> - ${data[5][1]}${(position == 6) ? "**" : ""}
${(position == 7) ? "**" : ""}#7 <@!${data[6][0]}> - ${data[6][1]}${(position == 7) ? "**" : ""}
${(position == 8) ? "**" : ""}#8 <@!${data[7][0]}> - ${data[7][1]}${(position == 8) ? "**" : ""}
${(position == 9) ? "**" : ""}#9 <@!${data[8][0]}> - ${data[8][1]}${(position == 9) ? "**" : ""}
${(position == 10) ? "**" : ""}#10 <@!${data[9][0]}> - ${data[9][1]}${(position == 10) ? "**" : ""}`
                
            if (position > 10) {
                string = `${string}\n. . .\n**#${position} <@!${data[position - 1][0]}> - ${data[position - 1][1]}**`
            }
        ...
        if (args.length == 0 || args[0].toLowerCase() == "elo") {
            ...
            let data = Object.entries(await db.get(`data`)).filter((a) => {return a[1].performance.wins + a[1].performance.losses >= 1})
            if (data.length < 10) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`Not enough leaderboard entries (${data.length})!`))
            ...
            let string = `${(position == 1) ? "**" : ""}#1 <@!${data[0][0]}> ${await db.get(`nameData.${data[0][0]}`)} - ${Math.round(data[0][1].rating)}${(position == 1) ? "**" : ""}
${(position == 2) ? "**" : ""}#2 <@!${data[1][0]}> ${await db.get(`nameData.${data[1][0]}`)} - ${Math.round(data[1][1].rating)}${(position == 2) ? "**" : ""}
${(position == 3) ? "**" : ""}#3 <@!${data[2][0]}> ${await db.get(`nameData.${data[2][0]}`)} - ${Math.round(data[2][1].rating)}${(position == 3) ? "**" : ""}
${(position == 4) ? "**" : ""}#4 <@!${data[3][0]}> ${await db.get(`nameData.${data[3][0]}`)} - ${Math.round(data[3][1].rating)}${(position == 4) ? "**" : ""}
${(position == 5) ? "**" : ""}#5 <@!${data[4][0]}> ${await db.get(`nameData.${data[4][0]}`)} - ${Math.round(data[4][1].rating)}${(position == 5) ? "**" : ""}
${(position == 6) ? "**" : ""}#6 <@!${data[5][0]}> ${await db.get(`nameData.${data[5][0]}`)} - ${Math.round(data[5][1].rating)}${(position == 6) ? "**" : ""}
${(position == 7) ? "**" : ""}#7 <@!${data[6][0]}> ${await db.get(`nameData.${data[6][0]}`)} - ${Math.round(data[6][1].rating)}${(position == 7) ? "**" : ""}
${(position == 8) ? "**" : ""}#8 <@!${data[7][0]}> ${await db.get(`nameData.${data[7][0]}`)} - ${Math.round(data[7][1].rating)}${(position == 8) ? "**" : ""}
${(position == 9) ? "**" : ""}#9 <@!${data[8][0]}> ${await db.get(`nameData.${data[8][0]}`)} - ${Math.round(data[8][1].rating)}${(position == 9) ? "**" : ""}
${(position == 10) ? "**" : ""}#10 <@!${data[9][0]}> ${await db.get(`nameData.${data[9][0]}`)} - ${Math.round(data[9][1].rating)}${(position == 10) ? "**" : ""}`
    
            if (position > 10) {
                string = `${string}\n. . .\n**#${position} <@!${data[position - 1][0]}> - ${Math.round(data[position - 1][1].rating)}**`
            }
            ...
            if (data.length < 10) return m.channel.send(new Discord.MessageEmbed()
                .setColor(embedColors.black)
                .setDescription(`Not enough leaderboard entries (${data.length})!`))
            data = data.sort((a, b) => {return b[1].performance.wins + b[1].performance.losses - a[1].performance.wins - a[1].performance.losses})
            ...
            let string = `${(position == 1) ? "**" : ""}#1 <@!${data[0][0]}> ${await db.get(`nameData.${data[0][0]}`)} - ${data[0][1].performance.wins + data[0][1].performance.losses}${(position == 1) ? "**" : ""}
${(position == 2) ? "**" : ""}#2 <@!${data[1][0]}> ${await db.get(`nameData.${data[1][0]}`)} - ${data[1][1].performance.wins + data[1][1].performance.losses}${(position == 2) ? "**" : ""}
${(position == 3) ? "**" : ""}#3 <@!${data[2][0]}> ${await db.get(`nameData.${data[2][0]}`)} - ${data[2][1].performance.wins + data[2][1].performance.losses}${(position == 3) ? "**" : ""}
${(position == 4) ? "**" : ""}#4 <@!${data[3][0]}> ${await db.get(`nameData.${data[3][0]}`)} - ${data[3][1].performance.wins + data[3][1].performance.losses}${(position == 4) ? "**" : ""}
${(position == 5) ? "**" : ""}#5 <@!${data[4][0]}> ${await db.get(`nameData.${data[4][0]}`)} - ${data[4][1].performance.wins + data[4][1].performance.losses}${(position == 5) ? "**" : ""}
${(position == 6) ? "**" : ""}#6 <@!${data[5][0]}> ${await db.get(`nameData.${data[5][0]}`)} - ${data[5][1].performance.wins + data[5][1].performance.losses}${(position == 6) ? "**" : ""}
${(position == 7) ? "**" : ""}#7 <@!${data[6][0]}> ${await db.get(`nameData.${data[6][0]}`)} - ${data[6][1].performance.wins + data[6][1].performance.losses}${(position == 7) ? "**" : ""}
${(position == 8) ? "**" : ""}#8 <@!${data[7][0]}> ${await db.get(`nameData.${data[7][0]}`)} - ${data[7][1].performance.wins + data[7][1].performance.losses}${(position == 8) ? "**" : ""}
${(position == 9) ? "**" : ""}#9 <@!${data[8][0]}> ${await db.get(`nameData.${data[8][0]}`)} - ${data[8][1].performance.wins + data[8][1].performance.losses}${(position == 9) ? "**" : ""}
${(position == 10) ? "**" : ""}#10 <@!${data[9][0]}> ${await db.get(`nameData.${data[9][0]}`)} - ${data[9][1].performance.wins + data[9][1].performance.losses}${(position == 10) ? "**" : ""}`
    
            if (position > 10) {
                string = `${string}\n. . .\n**#${position} <@!${data[position - 1][0]}> - ${data[position - 1][1].performance.wins + data[position - 1][1].performance.losses}**`
            }
        ...
        if (m.author.id in tagEloGames) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`You are in a game against <@!${getOpponent(m.author.id)}>. If you can't make it you must either forfeit (do ${prefix}forfeit <@!${getOpponent(m.author.id)}>) or abort (do ${prefix}abort)`))
        if (m.author.id in tagEloDuels) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
            .setDescription(`You are in a game against <@!${getOpponent(m.author.id)}>. If you can't make it you must either forfeit (do ${prefix}forfeit <@!${getOpponent(m.author.id)}>) or abort (do ${prefix}abort)`))
        if (m.author.id in tagEloQueues) return m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.black)
        ...
        if (pingedMemberID in tagEloGames) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        .setDescription("That player is already in a game"))
        if (pingedMemberID in tagEloDuels) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        .setDescription("That player is already in a game"))
        if (pingedMemberID in tagEloQueues) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        ...
        const msg = await m.channel.send(`<@!${pingedMemberID}>`, new Discord.MessageEmbed()
        .setColor(embedColors.blue)
        .setDescription(`React with a :thumbsup: if you would like to duel <@!${m.author.id}> (Rated ${Math.round(challenger.elo)})`))
        ...
        collector.on('end', async (collection, reason) => {
            msg.reactions.removeAll()
            if (reason == "success") {
                if (m.author.id in tagEloGames || pingedMemberID in tagEloGames || m.author.id in tagEloQueues || pingedMemberID in tagEloQueues || m.author.id in tagEloDuels || pingedMemberID in tagEloDuels) {
                ...
                .setColor(embedColors.blue)
                .setDescription(`Game ${await db.get(`matches`)} Starting:
<@!${m.author.id}> ${await db.get(`data.${m.author.id}.name`)} (${Math.round(player1.elo)}) vs <@!${pingedMemberID}> ${await db.get(`data.${pingedMemberID}.name`)} (${Math.round(player2.elo)})`)})
                // 
            }
            else if (reason == "failure") {
                return msg.edit({embed: new Discord.MessageEmbed()
        ...
        if (!(m.author.id in await db.get(`data`))) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        .setDescription("You must be verified first. Contact a Staff member for support"))

        if (m.author.id in tagEloQueues) {
            return m.author.send(new Discord.MessageEmbed()
                .setColor(embedColors.black)
                .setDescription("You must leave the queue to use the set command")).catch((err) => simulateDM(m, new Discord.MessageEmbed()
                .setColor(embedColors.black)
        ...
        if (m.member.hasPermission("ADMINISTRATOR")) return m.author.send({files:['./audit.txt']}).catch((err) => {return m.channel.send("Error while sending file (file too large?)")})
    }
...
If a <@&844040089310068777> hasn't responded within 15 minutes, you may both leave the server and freely queue`)
        ...
        if (!m.member.roles.cache.has(config.staffRoleID)) return
        if (m.mentions.members.size == 0) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        .setDescription("Ping the player you would like to award elo to"))

        let changeID = m.mentions.members.first().id

        if (!(changeID in await db.get(`data`))) return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.black)
        ...
        if (!(pingedID in tagEloGames) && !(pingedID in tagEloDuels)) return msg.edit({embed: new Discord.MessageEmbed()
            .setColor(embedColors.black)
        ...
        let newName = await hypixelFetch(`player?uuid=${await db.get(`data.${m.author.id}.uuid`)}`)
        ...
        msg = await m.channel.send(`<@!${getOpponent(m.author.id)}>`, new Discord.MessageEmbed()
            .setColor(embedColors.blue)
            .setDescription("React to this now if you're not afk (1/3)"))

        msg.react('✅')

        const filter = (reaction, user) => (user.id === getOpponent(m.author.id) && reaction.emoji.name == '✅')
        const collector = msg.createReactionCollector(filter, {time: 60 * 1000})
        ...
        collector.on('end', async (collection, reason) => {
            ...
            msg2 = await m.channel.send(`<@!${getOpponent(m.author.id)}>`, new Discord.MessageEmbed()
            .setColor(embedColors.blue)
            ...
            collector2.on('end', async (collection, reason) => {
                ...
                if (reason == "NOTAFK") {
                    return msg2.edit({embed: new Discord.MessageEmbed()
                    .setColor(embedColors.blue)
                    .setDescription("That player is not afk")})
                }
                ...
                msg3 = await m.channel.send(`<@!${getOpponent(m.author.id)}>`, new Discord.MessageEmbed()
                .setColor(embedColors.blue)
                ...
                collector3.on('end', async (collection, reason) => {
                    ...
                    if (reason == "NOTAFK") {
                        return msg3.edit({embed: new Discord.MessageEmbed()
                        .setColor(embedColors.blue)
                        .setDescription("That player is not afk")})
                    }
                    ...
                    if (!getOpponent(getOpponent(m.author.id))) return msg3.edit({embed: new Discord.MessageEmbed()
                        .setColor(embedColors.black)
                        .setDescription("Game End Failed! If this is an issue contact Mysterium")})
        ...
        let totalPlayers = (await Object.entries(await db.get(`data`)).filter((a) => {return a[1].performance.wins + a[1].performance.losses >= 1})).length
        let totalPlayers10 = (await Object.entries(await db.get(`data`)).filter((a) => {return a[1].performance.wins + a[1].performance.losses >= 10})).length
        m.channel.send(new Discord.MessageEmbed()
            .setColor(embedColors.blue)
...
**Total Players with at least 10 games:** ${totalPlayers10}`))
        ...
        addAudit(`${m.author.id} changed ${changeID}'s hostScore by ${args[1]} to ${await db.get(`hostData.${changeID}`)}`)
        return m.channel.send(new Discord.MessageEmbed()
        .setColor(embedColors.green)
        .setDescription(`Changed <@!${changeID}>'s hosting score to ${await db.get(`hostData.${changeID}`)}`))
...
client.on('clickButton', async (button) => {
    ...
    if (button.id.startsWith('register')) {
        ...
        if (args[1] == 'acc') {
            ...
            if (await db.has(`data.${args[2]}`)) return button.channel.send(`<@!${args[2]}>`, new Discord.MessageEmbed()
                .setColor(embedColors.black)
                .setDescription("That ID is already verified"))
            if (await Object.entries((await db.get('data'))).some((a) => {return a[1].uuid == args[4]})) return button.channel.send(`<@!${args[2]}>`, new Discord.MessageEmbed()
                .setColor(embedColors.black)
            ...
            return button.channel.send(`<@!${args[2]}>`, new Discord.MessageEmbed()
                .setColor(embedColors.green)
